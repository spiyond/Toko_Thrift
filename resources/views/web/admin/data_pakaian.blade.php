...
<table class="table align-middle table-bordered table-hover">
                    ...
                    @foreach ($data_pakaian as $items)
                        <tr>
                        <td>
                                {{ $items->pakaian_nama }}</td>
                                <td>{{ $items->pakaian_harga }}</td>
                                <td>{{ $items->pakaian_stok }}</td>
                                <a class="d-grid" href="{{route('data_pakaian.update', ['pakaian_id' => $items->pakaian_id])}}"><button class="btn btn-warning"><i class="bi bi-pencil-square"></i></button></a>
                            </td>
                            <td class="d-grid">
                                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deletePakaianModal_{{$items->pakaian_id}}">
                                    <i class="bi bi-trash3-fill"></i>
                                </button>
                            </td>
                            <div class="modal fade" id="deletePenulisModal_{{$items->pakaian_id}}" tabindex="-1" aria-labelledby="deletePenulisModalLabel_{{$items->pakaian_id}}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5" id="deletePenulisModalLabel_{{$items->pakaian_id}}">Konfirmasi Hapus</h1>
